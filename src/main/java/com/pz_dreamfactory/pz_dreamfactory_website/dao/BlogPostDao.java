...
public interface BlogPostDao extends JpaRepository<BlogPost, Integer> {
    ...
    /**
     * 通过 组别（int） 和 字段（String） 查询存活的博文目录
     * @param s1
     * @param group
     * @param pageable
     * @return
    ...
    @Query(nativeQuery = true,
            value = "select p.*, b.blogger_name from blog_post p, blogger b " +
                    ...
                    "\n#pageable\n")
    public Page<BlogPost> findAllByBloggerfIdAndAliveTrue(int id, Pageable pageable);
    ...
    @Query(nativeQuery = true,
            value = "select p.*, b.blogger_name from blog_post p, blogger b " +
                    "where b.bloger_fid = ?1 and p.blogger_fid = b.blogger_id;" +
                    "\n#pageable\n")
